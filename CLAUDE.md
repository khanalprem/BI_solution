...
| `/dashboard/deposits` | ✅ Live | Deposit Portfolio — driven by `public.get_deposit`. Dim chooser (11 dims: `year`, `year_quarter`, `year_month`, `tran_date`, `gam_province`, `gam_cluster`, `gam_branch`, `cif_id`, `acid`, `acct_num`, `acct_name`) → table with selected dims + `deposit` (SUM of `e.tran_date_bal`), paginated via `production/deposits`. Uses `useDashboardPage()` + `AdvancedFilters` for period/filter state. `Maturity Ladder` and `Cost of Funds` remain as `PlaceholderPanel` — those feeds are not part of `get_deposit`. |
...
- **Deposit Explorer architecture (`/dashboard/deposits`).** Driven by `public.get_deposit(select_clause, groupby_clause, partitionby_clause, orderby_clause, gam_where, date_where, date_join, branch_where, province_where, cluster_where, page, page_size)`. The procedure joins `gam g → eab e → dates d → branch b → province p → cluster c` and drops aggregated rows into a connection-scoped TEMP TABLE `deposit` with `deposit = SUM(e.tran_date_bal)` + `total_rows` + `rn` + `pivoted_totalrows`. Flow:
  ...
  - **Frontend** (`useDeposits` hook + `app/dashboard/deposits/page.tsx`): gated on `dimensions.length > 0`. Page is a 2-column layout (dim checklist left, results table right) — no measure selector (the procedure always emits `deposit`). Table renders selected dim columns + right-aligned `deposit` column (NPR-formatted); pagination UI mirrors the pivot page.
