...
- **Deposit Explorer architecture (`/dashboard/deposits`).** Driven by `public.get_deposit(select_clause, groupby_clause, partitionby_clause, orderby_clause, gam_where, date_where, date_join, branch_where, province_where, cluster_where, page, page_size)`. The procedure joins `gam g → eab e → dates d → branch b → province p → cluster c` and drops aggregated rows into a connection-scoped TEMP TABLE `deposit` with `deposit = SUM(e.tran_date_bal)` + `total_rows` + `rn` + `pivoted_totalrows`. Flow:
  - **Service** (`ProductionDataService#deposit_explorer`): builds each clause from `filters` + `dimensions`, calls the procedure, then `SELECT * FROM deposit` and strips `rn` / `total_rows` / `pivoted_totalrows` from the returned rows (they are pagination metadata, not user-visible columns).
  ...
  - **`date_where` cannot be empty.** If no tran-date filter is set the service falls back to `d.date BETWEEN start_date AND end_date` from the period selector. Exact values use `IN (...)`; single exact uses `=`; from/to uses `BETWEEN`.
