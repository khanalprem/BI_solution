...
| `/dashboard/deposits` | ✅ Live | Deposit Portfolio — driven by `public.get_deposit`. **Portfolio Overview** (3 live queries): Total Deposits / Branches Reporting / Top Branch Share / Period KPIs, Monthly Balance Trend line chart (`year_month` dim, ordered by `d.date ASC`), Top 10 Branches horizontal bar, Top 10 Depositors table (`cif_id + acct_name` dims). All overview queries snapshot to `filters.endDate` via a `tranDate` filter override so balances read as point-in-time, not window-sums. **Ad-hoc Breakdown** section below with the 11-dim checklist (`year`, `year_quarter`, `year_month`, `tran_date`, `gam_province`, `gam_cluster`, `gam_branch`, `cif_id`, `acid`, `acct_num`, `acct_name`) + paginated table via `production/deposits`. Default `page_size = 10` in all modes (including pivot) — matches the pivot page. The collapsible **`get_deposit` — All 12 Parameters** preview at the top of the section is built from local state (selected dims, partitionby_clause, orderby_clause, page, page_size) so users see the prepared CALL even before the procedure runs; WHERE clause strings are filled in from the response's `sql_preview` once the call returns, with a `<built server-side from active filters>` placeholder shown until then. Frontend mirrors of `DEPOSIT_DIMENSIONS[k][:sql]` and `:date_join` live in `deposits/page.tsx` as `DEPOSIT_DIM_SQL` / `DEPOSIT_DATE_JOIN` — keep them in sync if the backend dim list changes. The explorer query is gated on **at least one of the four date-dim filters being set** (`tranDate`, `yearMonth`, `yearQuarter`, `year` — exact, `_from`, or `_to`). When none is set the page suppresses the query and renders a "Date filter required" empty state in the results panel; the period selector alone is intentionally not enough because relying on its window almost always means scanning more data than the user wanted. `useDeposits` accepts an optional `{ enabled }` override so callers can layer on extra preconditions like this without affecting other deposit queries. Each non-date dim row has its own **ASC ↑ / DESC ↓** sort buttons (mirrors pivot page) — `orderby_clause` is empty by default and only assembled from the user's explicit sort selections (priority follows click order; sort badges show `sort #N ↓/↑`). Deselecting a dim drops any sort it carried so the ORDER BY token list never references a hidden dim. The four date dims (`year / year_quarter / year_month / tran_date`) share a **direction-selectable Sort** in the Date Dimensions container header — `dateSortDir: 'asc' | 'desc' | null`, rendered as a two-button group (↑ ASC and ↓ DESC) matching the per-non-date-dim sort UI. When set (and Pivot is off), all selected date dims auto-include in ORDER BY in fixed priority order (`tran_date → year_month → year_quarter → year`, finest-grain first) using that direction. Click the active direction again to turn off. Disabled while Pivot is on because the auto-pivot ORDER BY already covers the date dims. **Pivot-mode override:** when the date Pivot toggle is on, `orderbyClause` is auto-built as `ORDER BY <pivoted dates>, <other selected dims>` regardless of the user's manual Sort selections — pivoted dates have to sort first so each row's date columns line up left-to-right deterministically across pages, and the row identity is stabilised by the remaining non-date dims. Each dim row has an inline filter expand toggle (▼) — date dims expose Multi/Range modes (year YYYY, quarter YYYY-Qn, month YYYY-MM, date YYYY-MM-DD) wired to the matching DashboardFilters keys; categorical dims expose Single/Multi `SearchableMultiSelect`; `acct_name` uses a free-text chip input. Filter changes are AND-combined into `date_where` / `gam_where` / `branch_where` / `province_where` / `cluster_where` server-side. Date dims sit inside their own container with a shared **Pivot** toggle in the header — when on (and at least one date dim is checked), the page sends `partitionby_clause = "PARTITION BY <date dims>"` to `get_deposit`, bumps `page_size` to 200, and reshapes the response client-side into a pivot table where the date-dim values become column headers and non-date dims remain row labels (multi-date columns join with ` / ` for display). A collapsible **`get_deposit` — All 12 Parameters** panel below the results table renders a colour-coded `CALL public.get_deposit(...)` preview from `sql_preview` (toggle state `showProcPreview`) — same `SqlLine` / `KIND_CLS` styling as the pivot page's `get_tran_summary` preview. `Deposit Mix by Product`, `Cost of Funds`, `Maturity Ladder` remain as `PlaceholderPanel` — those feeds are not part of `get_deposit`. |
...
- **Placeholder pages pattern** (for features awaiting data-source integration): use `PlaceholderBanner` at the top of the page and `PlaceholderPanel` for individual "not connected" cards. Pages `/loans`, `/regulatory` follow this pattern with sector-average sample data. Sample constants are all named `SAMPLE_*` so replacing them with live feeds is a search-and-replace. Do NOT delete the sample constants until the replacement feed is tested — the placeholder UI relies on them to render the layout. (`/deposits` graduated off this pattern — see Deposit Explorer entry below.)
- **Deposit Explorer architecture (`/dashboard/deposits`).** Driven by `public.get_deposit(select_clause, groupby_clause, partitionby_clause, orderby_clause, gam_where, date_where, date_join, branch_where, province_where, cluster_where, page, page_size)`. The procedure joins `gam g → eab e → dates d → branch b → province p → cluster c` and drops aggregated rows into a connection-scoped TEMP TABLE `deposit` with `deposit = SUM(e.tran_date_bal)` + `total_rows` + `rn` + `pivoted_totalrows`. Flow:
  - **Service** (`ProductionDataService#deposit_explorer`): builds each clause from `filters` + `dimensions`, calls the procedure, then `SELECT * FROM deposit` and strips `rn` / `total_rows` / `pivoted_totalrows` from the returned rows (they are pagination metadata, not user-visible columns). **Alias substitution:** before passing `orderby_clause` and `partitionby_clause` to the procedure, the service runs `substitute_deposit_aliases` which rewrites bare dim keys (`year_month`, `gam_branch`, …) to their full SQL expressions (`d.year_month`, `b.branch_name`, …). The procedure applies these clauses inside `ROW_NUMBER() OVER(PARTITION BY … ORDER BY …)` in the same SELECT list where the alias is defined — Postgres can't resolve SELECT-list aliases in that position, so passing bare keys raises `column "year_month" does not exist`. This mirrors the pivot service's measure-alias substitution; keep the helper in sync if `DEPOSIT_DIMENSIONS[k][:sql]` ever changes.
  - **`DEPOSIT_DIMENSIONS` hash** (service constant) — the 11 allowed dims: `gam_branch → b.branch_name`, `gam_province → p.name`, `gam_cluster → c.cluster_name`, `acct_num/acct_name/acid/cif_id → g.<col>`, and the 4 date dims `tran_date/year_month/year_quarter/year → d.<col>`. Each date dim carries its `date_join` string (empty for `tran_date`; `AND d.date = d.month_enddate` etc. for the others). `DEPOSIT_DATE_DIM_PRIORITY = [tran_date, year_month, year_quarter, year]` — finest granularity wins if multiple are selected, mirroring the pivot-explorer EAB as-of logic.
  - **WHERE clauses** (all prefixed with `AND` when non-empty because the procedure template already has a leading `WHERE 1=1`; `date_where` is the sole exception — it is the full condition without `AND` since the procedure concatenates it directly). `gam_where` filters `acct_num / acid / cif_id / acct_name`; `branch_where` filters `g.sol_id` (both `branch` and `solid` filter keys map here); `province_where` filters `p.name`; `cluster_where` filters `b.cluster_id`.
  - **`date_where` cannot be empty.** Built by `build_deposit_date_where` (driven by the `DEPOSIT_DATE_FILTER_COLS` map). All four date-dim filters — `tran_date / year / year_month / year_quarter` — are honored, each combined with `AND`. Per-column behavior: exact-multi → `IN (...)`; exact-single → `=`; both `_from`/`_to` set → `BETWEEN`; one-sided range → `>=` or `<=`. If none of the four are set the service falls back to `d.date BETWEEN start_date AND end_date` from the period selector so the procedure never receives an empty predicate. The frontend mirrors this builder as `buildClientDateWhere(filters)` (driven by `DEPOSIT_DATE_WHERE_COLS`) so the SQL preview can show the actual predicate even before the procedure has been called — keep both lists in sync if the date filter set ever changes.
  - **Per-dim inline filters in the UI.** The deposit page's dim sidebar mirrors the pivot page: each dim row has an expand toggle (▼) that opens an inline filter editor — date dims get a Multi/Range mode (multi = comma-separated chip text input, range = From/To inputs); categorical dims (`gam_branch / gam_province / gam_cluster / cif_id / acid / acct_num`) get a Single/Multi `SearchableMultiSelect` populated from `useFilterValues` (`/filters/values`); `acct_name` uses `MultiValueChipInput` (free-text chips). State lives in `expandedDim`, `dateFilterModes`, `categoricalFilterModes`. Filters are written back via `setFieldFilter` which also resets `explorerPage` to 1. A `filtered` badge renders next to the dim label whenever any of its filter keys hold a value.
  - **Controller** (`ProductionController#deposits`): parses dims + filters, runs `partitionby_clause` and `orderby_clause` through `sanitize_deposit_clause` (tighter whitelist than the pivot one — only `DEPOSIT_DIMENSIONS.keys + [deposit, tran_date_bal, e]` and the SUM function).
  - **Frontend** (`useDeposits` hook + `app/dashboard/deposits/page.tsx`). The page fires **three live overview queries** + one user-driven ad-hoc query, all against the same `useDeposits` hook (each has a unique queryKey via `dimensions`):
    1. **Branch snapshot** — `dims=['gam_branch']`, `tranDate` overridden to `filters.endDate` (so `date_where = "d.date = '<endDate>'"`), `page_size=200`. Drives Total Deposits KPI (sum of rows), Branches Reporting KPI, Top Branch Share KPI, and the Top 10 Branches horizontal bar chart.
    2. **Monthly trend** — `dims=['year_month']`, full date window, `orderby_clause='ORDER BY d.date ASC'`, `page_size=200`. Drives the Monthly Balance Trend line chart (one row per month-end via `d.date = d.month_enddate` join).
    3. **Top depositors snapshot** — `dims=['cif_id','acct_name']`, same `tranDate=endDate` override, `page_size=10`, default `ORDER BY sum(e.tran_date_bal) DESC`. Drives the Top 10 Depositors table.
    - **Snapshot filter pattern** — a `snapshotFilters` memo clones `filters` and sets `tranDate = filters.endDate` + clears `tranDateFrom`/`tranDateTo`. This pins the backend `date_where` to a single day so the KPI values read as point-in-time balances (not summed across the window, which would double-count daily snapshots). Passed into the branch + top-depositor queries; the trend query uses the un-overridden `filters` because it explicitly wants the full window.
    - **Ad-hoc breakdown section** below the overview reuses the 11-dim checklist (dims in `DEPOSIT_DIMS`) + paginated `useDeposits` query. Gated on `dimensions.length > 0`. The `dimsMatch` memo discards stale `placeholderData` when the user toggles dims so newly-added columns never show `—` from the previous query's schema.
