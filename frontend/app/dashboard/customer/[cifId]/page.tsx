...
export default function CustomerDetailPage() {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Link href={`/dashboard/branch/${encodeURIComponent(row.original.branch_code)}`} className="font-medium text-accent-blue hover:underline">
  ...
  const accountColumns = useMemo<ColumnDef<Record<string, string | number | boolean | null>>[]>(
    () => [
      { accessorKey: 'acct_num',       header: 'Account Number', cell: ({ row }) => <strong className="text-text-primary">{String(row.original.acct_num ?? '')}</strong>, enableColumnFilter: true },
      { accessorKey: 'acid',           header: 'ACID',           enableColumnFilter: true },
      { accessorKey: 'acct_name',      header: 'Account Name',   cell: ({ row }) => String(row.original.acct_name || '—'), enableColumnFilter: true },
      { accessorKey: 'schm_type',      header: 'Scheme Type',    enableColumnFilter: true, filterFn: 'arrayFilter', meta: { filterType: 'select' } },
      { accessorKey: 'sol_id',         header: 'SOL ID',         enableColumnFilter: true },
      { accessorKey: 'gl_sub_head_code', header: 'GL Code',      enableColumnFilter: true },
      { accessorKey: 'eod_balance',    header: 'EOD Balance',    cell: ({ row }) => <strong className="text-text-primary">{formatNPR(Number(row.original.eod_balance ?? 0))}</strong>, enableSorting: true, sortDescFirst: true },
      { accessorKey: 'lchg_time',      header: 'Last Updated',   cell: ({ row }) => row.original.lchg_time ? new Date(String(row.original.lchg_time)).toLocaleDateString('en-NP', { year: 'numeric', month: 'short', day: 'numeric' }) : '—' },
    ],
    []
  );
  ...
  const totalAccountBalance = accounts.reduce((sum, account) => sum + Number(account.eod_balance ?? 0), 0);
      ...
      <div className="px-5 py-4 flex flex-col gap-[14px]">
          ...
          <div className="rounded-xl border border-border bg-bg-card p-4">
            <div className="flex items-start gap-4">
              {/* Avatar */}
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-white text-[16px] font-bold flex-shrink-0"
                ...
                <div className="mt-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-2">
                  {[
                    ...
                    <div key={item.label}>
                      ...
                      <div className="text-[11.5px] text-text-primary mt-0.5 truncate">{item.value ?? '—'}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <RecordTable
            ...
            columns={(profile as any)?.account_columns || Object.keys(accounts[0] || {})}
            rows={accounts as unknown as Array<Record<string, string | number | boolean | null>>}
          />
