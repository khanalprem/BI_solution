        ...
        {glData.length > 0 && (() => {
          ...
          const glColumns: ColumnDef<GlRow>[] = [
            ...
            { accessorKey: 'debit', header: 'Debit (DR)', enableSorting: true, sortDescFirst: true,
              enableColumnFilter: true, filterFn: 'numberRange', meta: { filterType: 'number-range' },
              cell: ({ row }) => <span className="font-mono text-xs text-accent-red">{formatNPR(row.original.debit)}</span> },
            { accessorKey: 'net', header: 'Net Flow', enableSorting: true,
              cell: ({ row }) => <span className={`font-mono text-xs ${row.original.net >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>{formatNPR(row.original.net)}</span> },
            { id: 'total', header: 'Total Volume', enableSorting: true,
              ...
              cell: ({ row }) => <span className="font-mono text-xs">{formatNPR(row.original.credit + row.original.debit)}</span> },
