...
export default function EmployeeDetailPage() {
  ...
  const empBranchTotal = useMemo(
    () => byBranch.reduce((s, b) => s + b.amount, 0),
    [byBranch],
  );
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Link
            href={`/dashboard/branch/${encodeURIComponent(row.original.branch)}`}
      ...
      },
      {
        ...
        enableSorting: true,
        sortingFn: (a, b) =>
          (a.original.count > 0 ? a.original.amount / a.original.count : 0) -
          (b.original.count > 0 ? b.original.amount / b.original.count : 0),
        cell: ({ row }) => (
          <span className="font-mono text-xs">
            {row.original.count > 0
              ? formatNPR(row.original.amount / row.original.count)
              : "—"}
          </span>
        ),
      },
      {
        ...
        enableSorting: true,
        sortingFn: (a, b) =>
          (a.original.accounts > 0
            ? a.original.count / a.original.accounts
            : 0) -
          (b.original.accounts > 0
            ? b.original.count / b.original.accounts
            : 0),
        cell: ({ row }) =>
          row.original.accounts > 0
            ? (row.original.count / row.original.accounts).toFixed(1)
            : "—",
      },
      {
        id: "vol_per_account",
        ...
        sortingFn: (a, b) =>
          (a.original.accounts > 0
            ? a.original.amount / a.original.accounts
            : 0) -
          (b.original.accounts > 0
            ? b.original.amount / b.original.accounts
            : 0),
        cell: ({ row }) => (
          <span className="font-mono text-xs">
            {row.original.accounts > 0
              ? formatNPR(row.original.amount / row.original.accounts)
              : "—"}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const pct =
            empBranchTotal > 0
              ? (row.original.amount / empBranchTotal) * 100
              : 0;
          return (
            <div className="flex items-center gap-2 min-w-[70px]">
              ...
              <span className="text-[9.5px] text-text-muted w-8 text-right">
                {pct.toFixed(1)}%
              </span>
            </div>
          );
  ...
  const productTotal = useMemo(
    () => byProduct.reduce((s, p) => s + p.amount, 0),
    [byProduct],
  );
  const productColumns = useMemo<ColumnDef<ProductRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const pct =
            productTotal > 0 ? (row.original.amount / productTotal) * 100 : 0;
          return (
            <div className="flex items-center gap-2 min-w-[70px]">
              <div className="flex-1 h-1.5 rounded-full bg-bg-input overflow-hidden">
                <div
      ...
      <div className="flex flex-col gap-[14px] px-5 py-4 page-enter">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[14px] font-semibold font-display tracking-tight">
                {userId}
              </div>
              <div className="text-[10px] text-text-muted">
                Entry User · {data?.active_branches ?? 0} branch
                {(data?.active_branches ?? 0) !== 1 ? "es" : ""} active
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-bg-card border border-border rounded-xl p-4">
            ...
            <div className="text-2xl font-mono font-bold tracking-tight text-accent-green font-mono">
          ...
          <div className="bg-bg-card border border-border rounded-xl p-4">
            ...
            <div className="text-2xl font-mono font-bold tracking-tight text-accent-red font-mono">
          ...
          <div className="bg-bg-card border border-border rounded-xl p-4">
            ...
            <div className="text-2xl font-mono font-bold tracking-tight text-accent-amber font-mono">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {monthlyTrend.length > 0 && (
            <ChartCard
              ...
              subtitle="Credit vs Debit by month"
            >
              <PremiumLineChart
                data={monthlyTrend}
                xAxisKey="month"
          ...
          {byBranch.length > 0 && (
            <ChartCard
              ...
              subtitle="Volume by branch handled"
            >
              <PremiumBarChart
                data={byBranch.slice(0, 8)}
                xAxisKey="branch"
                series={[
                  { dataKey: "amount", name: "Volume", color: "#3b82f6" },
                ]}
                layout="horizontal"
                formatValue={formatNPR}
                yAxisWidth={80}
        ...
        {dailyTrend.length > 0 && (
          <ChartCard
            ...
            subtitle="Transaction volume per day"
          >
            <PremiumLineChart
              data={dailyTrend}
              xAxisKey="date"
        ...
        {byBranch.length > 0 && (
          <AdvancedDataTable
            ...
            pageSize={10}
            initialHidden={{
              txn_per_account: true,
              vol_per_account: true,
              share: true,
            }}
          />
        )}
